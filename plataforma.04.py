...
@app.route('/cadastrar_Catalago', methods=['GET', 'POST'])
def cadastrar_plano():
    if request.method == "POST":
        nome = request.form['nome']
    ...
    return render_template("catalogo-cadastrar.html", titulo=vartitulo)

@app.route('/listar_Catalogo', methods=['GET',])
def listar_Catalogo():
    dao = CatalogoDAO(get_db())
    catalogo_db = dao.listar()
    return render_template("catalogo-listar.html", catalogo=catalogo_db)
